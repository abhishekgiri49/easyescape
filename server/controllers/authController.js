...
const registerUser = async (req, res) => {
    try {
        ...
        // Create a new user
        const newUser = new User({ name,username,email,role:'user', password: hashedPassword });
        await newUser.save();

        res.status(201).json({status:201, message: 'User registered successfully',data:[] });
    } catch (error) {
        console.error(error);
...
const loginUser = async (req, res) => {
    try {
        ...
        const token = jwt.sign({ userId: user._id }, 'runner', { expiresIn: '24h' });
        const result = {
            token: token,
            user: user
          };
