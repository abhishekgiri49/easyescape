...
const getItemById = async (req, res) => {
  try {
    ...
    if (!blog) return res.status(404).json({ status:404, data:[],message: 'Blog not found' });
...
const updateItemById = async (req, res) => {
  try {
    ...
    if (!updatedBlog) return res.status(404).json({ status:404, data:[],message: 'Blog not found' });
    res.json({status:200, data:updatedBlog, message: 'Blog updated successfully' });
  } catch (error) {
    console.error(error);
    res.status(500).json({status:500, data:[], message: 'Internal Server Error' });
  }
...
const deleteItemById = async (req, res) => {
  try {
    ...
    if (!deletedBlog) return res.status(404).json({status:404, data:[], message: 'Blog not found' });
